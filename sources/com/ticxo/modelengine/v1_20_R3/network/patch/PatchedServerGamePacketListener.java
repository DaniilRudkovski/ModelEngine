 ...
 *  org.bukkit.craftbukkit.v1_20_R3.event.CraftEventFactory
...
public class PatchedServerGamePacketListener {
    ...
    public static void handleInteract(final ServerboundInteractPacketWrapper interactPacket, PacketListenerPlayIn listener) {
        ...
        PlayerConnectionUtils.a((Packet)interactPacket, (PacketListener)connection, (WorldServer)connection.p().z());
        final EntityPlayer player = connection.p();
        if (player.eY()) {
            return;
        }
        final CraftPlayer craftPlayer = player.getBukkitEntity();
        final WorldServer serverLevel = player.z();
        final CraftServer craftServer = serverLevel.o().server;
        ...
        if (!serverLevel.D_().a(entity.dm())) {
            return;
        }
        AxisAlignedBB boundingBox = entity.cH();
        if (boundingBox.e(player.br()) >= PlayerConnection.d) {
            return;
        }
        interactPacket.a(new PacketPlayInUseEntity.c(){
            ...
            private void performInteraction(EnumHand enumhand, EntityInteraction entityInteraction, PlayerInteractEntityEvent event) {
                ...
                if (itemstack.a(serverLevel.I())) {
                    ItemStack itemstack1 = itemstack.p();
                    ItemStack itemInHand = player.b(enumhand);
                    boolean triggerLeashUpdate = itemInHand.d() == Items.uD && entity instanceof EntityInsentient;
                    Item origItem = player.fS().f().d();
                    craftServer.getPluginManager().callEvent((Event)event);
                    if (entity instanceof Bucketable && entity instanceof EntityLiving && origItem.k() == Items.qw && (event.isCancelled() || player.fS().f().d() != origItem)) {
                        entity.an().resendPossiblyDesyncedEntity(player);
                        player.bS.b();
                    }
                    if (triggerLeashUpdate && (event.isCancelled() || player.fS().f().d() != origItem)) {
                        connection.b((Packet)new PacketPlayOutAttachEntity(entity, ((EntityInsentient)entity).fT()));
                    }
                    if (event.isCancelled() || player.fS().f().d() != origItem) {
                        entity.an().refresh(player);
                        if (entity instanceof Allay) {
                            Allay allay = (Allay)entity;
                            connection.b((Packet)new PacketPlayOutEntityEquipment(entity.aj(), Arrays.stream(EnumItemSlot.values()).map(slot -> Pair.of((Object)slot, (Object)allay.stripMeta(allay.c(slot), true))).collect(Collectors.toList())));
                            player.bS.b();
                        }
                    }
                    ...
                    if (enuminteractionresult.a()) {
                        CriterionTriggers.R.a(player, itemstack1, entity);
            ...
            public void a() {
                ItemStack itemstack;
                if (!(entity instanceof EntityItem || entity instanceof EntityExperienceOrb || entity instanceof EntityArrow || entity == player && !player.P_() || !(itemstack = player.b(EnumHand.a)).a(serverLevel.I()))) {
                    player.d(entity);
    ...
    public static void handleUseItem(PacketPlayInBlockPlace packet, PacketListenerPlayIn listener, Consumer<EnumInteractionResult> afterUse) {
        ...
        PlayerConnectionUtils.a((Packet)packet, (PacketListener)connection, (WorldServer)player.z());
        if (player.eY()) {
            return;
        }
        ...
        WorldServer worldserver = player.z();
        EnumHand enumhand = packet.a();
        ItemStack itemstack = player.b(enumhand);
        player.G();
        if (!itemstack.b() && itemstack.a(worldserver.I())) {
            boolean cancelled;
            float f1 = player.dE();
            float f2 = player.dC();
            double d0 = player.dr();
            double d1 = player.dt() + (double)player.cI();
            double d2 = player.dx();
            ...
            MovingObjectPositionBlock movingobjectposition = player.dM().a(new RayTrace(vec3d, vec3d1, RayTrace.BlockCollisionOption.b, RayTrace.FluidCollisionOption.a, (net.minecraft.world.entity.Entity)player));
