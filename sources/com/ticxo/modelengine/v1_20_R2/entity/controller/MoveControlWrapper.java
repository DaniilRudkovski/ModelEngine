 ...
 *  org.bukkit.craftbukkit.v1_20_R2.util.CraftVector
...
implements MoveController {
    ...
    public void a() {
        ...
        if (!(rootMotion == null || (velocity = rootMotion.delta()).isZero() && rootMotion.onGround())) {
            ...
            if (!TMath.isSimilar((float)velocity.getY(), 0.0f) || !rootMotion.onGround()) {
                this.d.f(this.d.do().d(1.0, 0.0, 1.0));
            }
    ...
    private void fromVector(Vector vector) {
        Vec3D vec = new Vec3D(vector.getX(), vector.getY(), vector.getZ()).b(-this.d.dB() * ((float)Math.PI / 180));
        this.d.C((float)vec.c);
        this.d.B((float)vec.d);
        this.d.A((float)vec.e);
    }
    ...
    private Vector toVector() {
        return new Vector(Float.isNaN(this.d.bk) ? 0.0f : this.d.bk, Float.isNaN(this.d.bl) ? 0.0f : this.d.bl, Float.isNaN(this.d.bm) ? 0.0f : this.d.bm).rotateAroundY((double)(this.d.dB() * ((float)Math.PI / 180)));
    }
