 ...
 *  org.joml.Vector3f
 */
package com.ticxo.modelengine.v1_21_R5.entity.hitbox;
...
import com.ticxo.modelengine.v1_21_R5.entity.hitbox.OBB;
...
implements HitboxEntity {
    ...
    public boolean hurtServer(@NotNull ServerLevel world, @NotNull DamageSource source, float amount) {
        ...
        for (Entity entity : this.subHitbox.getBoundEntities().values()) {
            EntityUtils.nms(entity).hurtServer(world, source, amount);
        }
        if (this.subHitbox.getDamageMultiplier() <= 1.0E-5f) {
            return false;
        }
        BaseEntity<?> baseEntity = this.bone.getActiveModel().getModeledEntity().getBase();
        ...
        return baseEntity.hurt(this, (HumanEntity)cause, source, amount * this.subHitbox.getDamageMultiplier());
    }
    ...
    public InteractionResult interact(Player player, @NotNull InteractionHand hand) {
        ...
        if (craftHumanEntity instanceof org.bukkit.entity.Player) {
            ...
            for (Entity entity : this.subHitbox.getBoundEntities().values()) {
                PlayerInteractAtEntityEvent event = new PlayerInteractAtEntityEvent(craftPlayer, entity, new Vector(0, 0, 0), hand == InteractionHand.OFF_HAND ? EquipmentSlot.OFF_HAND : EquipmentSlot.HAND);
        ...
        EntityHandler.InteractionResult result = this.bone.getActiveModel().getModeledEntity().getBase().interact(this, (HumanEntity)player.getBukkitEntity(), hand == InteractionHand.MAIN_HAND ? EquipmentSlot.HAND : EquipmentSlot.OFF_HAND);
        return switch (result) {
            default -> throw new MatchException(null, null);
            case EntityHandler.InteractionResult.SUCCESS -> InteractionResult.SUCCESS_SERVER;
            case EntityHandler.InteractionResult.SUCCESS_NO_ITEM_USED -> InteractionResult.TRY_WITH_EMPTY_HAND;
            case EntityHandler.InteractionResult.CONSUME -> InteractionResult.CONSUME;
            case EntityHandler.InteractionResult.CONSUME_PARTIAL -> InteractionResult.SUCCESS;
