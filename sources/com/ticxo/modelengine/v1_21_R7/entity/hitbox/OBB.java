...
extends AABB {
    ...
    public Optional<Vec3> clip(Vec3 from, Vec3 to) {
        ...
        Vec3 rFrom = new Vec3((Vector3fc)from.subtract(center).toVector3f().rotateY(-yaw).rotate((Quaternionfc)inverse)).add(center);
        Optional result = super.clip(rFrom, rTo = new Vec3((Vector3fc)to.subtract(center).toVector3f().rotateY(-yaw).rotate((Quaternionfc)inverse)).add(center));
        if (result.isEmpty()) {
            return result;
        }
        Vec3 clip = new Vec3((Vector3fc)((Vec3)result.get()).subtract(center).toVector3f().rotate((Quaternionfc)this.rotation).rotateY(yaw)).add(center);
