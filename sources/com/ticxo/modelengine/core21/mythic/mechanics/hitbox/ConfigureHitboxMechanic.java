...
public class ConfigureHitboxMechanic
implements ITargetedEntitySkill {
    private final PlaceholderString modelId;
    private final PlaceholderString partId;
    private final PlaceholderDouble shouldPass;

    public ConfigureHitboxMechanic(MythicLineConfig mlc) {
        ...
        this.shouldPass = mlc.getPlaceholderDouble(new String[]{"pass"}, 1.0, new String[0]);
    }
    ...
    public SkillResult castAtEntity(SkillMetadata meta, AbstractEntity target) {
        ...
        model.getModel(modelId).ifPresent(activeModel -> {
            ...
            activeModel.getBone(partId).flatMap(modelBone -> modelBone.getBoneBehavior(BoneBehaviorTypes.SUB_HITBOX)).ifPresent(subHitbox -> ((SubHitbox)((Object)subHitbox)).setDamageMultiplier((float)this.shouldPass.get((PlaceholderMeta)meta, target)));
        });
