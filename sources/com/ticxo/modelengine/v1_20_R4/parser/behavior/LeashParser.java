...
implements BehaviorRendererParser<LeashRenderer> {
    ...
    private ClientboundAddEntityPacket leashSpawn(LeashRenderer.Leash renderer) {
        Vector3f pos = renderer.getPosition().get();
        return new ClientboundAddEntityPacket(renderer.getLeashId(), renderer.getLeastUUID(), (double)pos.x, (double)pos.y, (double)pos.z, 0.0f, 0.0f, EntityType.SLIME, 0, Vec3.ZERO, 0.0);
    }
    ...
    private ClientboundSetEntityDataPacket leashData(LeashRenderer.Leash renderer) {
        return new ClientboundSetEntityDataPacket(renderer.getLeashId(), EntityUtils.LEASH_SLIME_DATA);
    }
