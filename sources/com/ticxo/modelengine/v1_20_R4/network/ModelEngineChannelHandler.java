...
extends ChannelDuplexHandler {
    ...
    public ModelEngineChannelHandler(Player player, PipelineWrapper pipeline) {
        ...
        this.readInterceptors.register(ServerboundInteractPacket.class, this::handleInteract).register(ServerboundPlayerInputPacket.class, this::handlePlayerInput).register(ServerboundPongPacket.class, this::handlePong).register(ServerboundMovePlayerPacket.Pos.class, this::handlePlayerMove).register(ServerboundMovePlayerPacket.PosRot.class, this::handlePlayerMove);
    }
    ...
    private List<Packet<? super ClientGamePacketListener>> handleMount(UUID uuid) {
        ...
        model.getBehaviorRenderer(BoneBehaviorTypes.MOUNT).ifPresent(behaviorRenderer -> {
            if (behaviorRenderer instanceof MountRenderer) {
                ...
                Mount patt0$temp = controller.getMount();
                if (patt0$temp instanceof BoneBehavior) {
                    BoneBehavior behavior = (BoneBehavior)((Object)patt0$temp);
                    ...
                    list.add((Packet<? super ClientGamePacketListener>)new ClientboundSetPassengersPacket(EntityContainer.of(mount.getMountId(), ids)));
                }
            }
        });
        return list;
    ...
    private ServerboundPlayerInputPacket handlePlayerInput(ServerboundPlayerInputPacket inputPacket) {
        ...
        if (controller != null) {
            ...
            if (input == null) {
                controller.setInput(new MountController.MountInput(inputPacket.getXxa(), inputPacket.getZza(), inputPacket.isJumping(), inputPacket.isShiftKeyDown()));
            } else {
                input.setSide(Float.valueOf(inputPacket.getXxa()));
                input.setFront(Float.valueOf(inputPacket.getZza()));
                input.setJump(inputPacket.isJumping());
                input.setSneak(inputPacket.isShiftKeyDown());
            }
        }
        return inputPacket;
    }
