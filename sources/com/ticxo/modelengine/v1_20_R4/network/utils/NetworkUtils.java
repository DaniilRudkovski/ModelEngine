...
public class NetworkUtils {
    public static final ProtocolInfo<ClientGamePacketListener> CLIENTBOUND_CODEC = GameProtocols.CLIENTBOUND.bind(RegistryFriendlyByteBuf.decorator((RegistryAccess)MinecraftServer.getServer().registryAccess()));
    public static final ProtocolInfo<ServerGamePacketListener> SERVERBOUND_CODEC = GameProtocols.SERVERBOUND.bind(RegistryFriendlyByteBuf.decorator((RegistryAccess)MinecraftServer.getServer().registryAccess()));
    ...
    public static FriendlyByteBuf createByteBuf(Consumer<FriendlyByteBuf> consumer) {
        FriendlyByteBuf buf = NetworkUtils.createByteBuf();
        consumer.accept(buf);
        return buf;
    }

    public static <T extends Packet<ClientGamePacketListener>> T create(Class<T> clazz, FriendlyByteBuf buf) {
    ...
    public static Packets.PacketSupplier createPivotTeleport(int id, Vector3f pos) {
        ...
        ClientboundTeleportEntityPacket tpHigher = new ClientboundTeleportEntityPacket((Entity)container);
        container = EntityContainer.of(id);
        container.setId(id);
        container.setPosRaw(pos.x, (double)pos.y - 0.5, pos.z);
        ClientboundTeleportEntityPacket tpLower = new ClientboundTeleportEntityPacket((Entity)container);
    ...
    public static void sendBundled(UUID target, Packets collection) {
        ...
        Collection<Packet<? super ClientGamePacketListener>> packets = collection.compile(target);
        NetworkHandlerImpl handler = NetworkHandlerImpl.instance;
        if (handler.isBatching()) {
