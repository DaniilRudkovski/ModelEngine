...
implements TrackedEntity {
    ...
    private final Supplier<ChunkMap.TrackedEntity> trackedEntitySupplier;
    ...
    @NotNull
    private ChunkMap.TrackedEntity lastTrackedEntity;

    public TrackedEntityImpl(Entity entity, Supplier<ChunkMap.TrackedEntity> trackedEntitySupplier, @NotNull ChunkMap.TrackedEntity lastTrackedEntity) {
        this.entity = entity;
    ...
    public Set<UUID> getTrackedPlayer() {
        ...
        for (ServerPlayerConnection connection : this.getTrackedEntity().seenBy) {
            Player player = Bukkit.getPlayer((UUID)connection.getPlayer().getUUID());
    ...
    public void sendPairingData(Player player) {
        ...
        this.getTrackedEntity().serverEntity.sendPairingData(nms, packet -> NetworkUtils.send(player.getUniqueId(), (Packet<? super ClientGamePacketListener>)packet));
    }
    ...
    public void broadcastRemove() {
        this.getTrackedEntity().broadcastRemoved();
    }
    ...
    @NotNull
    private ChunkMap.TrackedEntity getTrackedEntity() {
        ChunkMap.TrackedEntity trackedEntity = this.trackedEntitySupplier.get();
    ...
    private void syncInstance(ChunkMap.TrackedEntity target) {
