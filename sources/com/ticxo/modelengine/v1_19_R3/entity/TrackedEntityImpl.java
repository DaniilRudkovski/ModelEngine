...
implements TrackedEntity {
    ...
    public Set<UUID> getTrackedPlayer() {
        ...
        for (ServerPlayerConnection connection : this.getTrackedEntity().f) {
            Player player = Bukkit.getPlayer((UUID)connection.f().cs());
    ...
    public void sendPairingData(Player player) {
        ...
        this.getTrackedEntity().b.sendPairingData(packet -> NetworkUtils.send(player.getUniqueId(), (Packet<? super PacketListenerPlayOut>)packet), nms);
    }
