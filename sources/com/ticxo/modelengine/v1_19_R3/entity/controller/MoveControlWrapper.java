 ...
 *  org.bukkit.craftbukkit.v1_19_R3.util.CraftVector
...
implements MoveController {
    ...
    public void a() {
        ...
        this.isOnGround = this.d.ax();
        ModeledEntity modeledEntity = ModelEngineAPI.getModeledEntity(this.d.cs());
        ...
        if (!(rootMotion == null || (velocity = rootMotion.delta()).isZero() && rootMotion.onGround())) {
            ...
            if (!TMath.isSimilar((float)velocity.getY(), 0.0f) || !rootMotion.onGround()) {
                this.d.f(this.d.dj().d(1.0, 0.0, 1.0));
            }
    ...
    public void globalMove(float x, float y, float z, float speedModifier) {
        float speed = this.getSpeed();
        this.d.h(speed * speedModifier);
        Vec3D vec = new Vec3D((double)x, (double)y, (double)z).b(-this.d.dw() * ((float)Math.PI / 180));
        this.d.A((float)vec.c);
        this.d.z((float)vec.d);
        this.d.y((float)vec.e);
    }

    private void fromVector(Vector vector) {
        Vec3D vec = new Vec3D(vector.getX(), vector.getY(), vector.getZ()).b(-this.d.dw() * ((float)Math.PI / 180));
        this.d.A((float)vec.c);
        this.d.z((float)vec.d);
        this.d.y((float)vec.e);
    }
    ...
    private Vector toVector() {
        return new Vector(Float.isNaN(this.d.bj) ? 0.0f : this.d.bj, Float.isNaN(this.d.bk) ? 0.0f : this.d.bk, Float.isNaN(this.d.bl) ? 0.0f : this.d.bl).rotateAroundY((double)(this.d.dw() * ((float)Math.PI / 180)));
    }
    ...
    public Vector getVelocity() {
        return CraftVector.toBukkit((Vec3D)this.d.dj());
    }
