...
implements BehaviorRendererParser<NameTagRenderer> {
    ...
    private PacketPlayOutSpawnEntity tagSpawn(NameTagRenderer.NameTag renderer) {
        ...
        return new PacketPlayOutSpawnEntity(renderer.getTagId(), renderer.getTagUuid(), (double)pos.x, (double)pos.y, (double)pos.z, 0.0f, 0.0f, EntityTypes.aX, 0, Vec3D.b, 0.0);
    }
    ...
    private PacketPlayOutEntityMetadata tagData(NameTagRenderer.NameTag renderer, boolean spawn) {
        ...
        renderer.getVisibility().ifDirty(flag -> EntityUtils.writeData(buf, 16, DataWatcherRegistry.d, Float.valueOf(flag != false ? 4096.0f : 0.0f)), spawn);
        ...
        renderer.getBillboard().ifDirty(flag -> {
            ...
            EntityUtils.writeData(buf, 14, DataWatcherRegistry.a, value);
        }, spawn);
        renderer.getJsonString().ifDirty(jsonString -> {
            if (jsonString == null) {
                EntityUtils.writeData(buf, 23, DataWatcherRegistry.f, IChatBaseComponent.h());
            } else {
